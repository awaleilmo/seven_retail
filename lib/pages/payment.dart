...
class _Payment extends State<Payment> {
  ...
  void save() async {
    ...
    final rawJsons1 = prefs.getString('order');
    var dataBill = rawJsons != null ? jsonDecode(rawJsons) : [],
        dataOrder = rawJsons1 != null ? jsonDecode(rawJsons1) : [],
        news = [];
    String? pyment = '';
    for (int i = 0; i < pay.length; i++) {
      if (pay[i]['value'] == pilih) {
        pyment = pay[i]['name'].toString();
      }
    }
    var data = {
      "id": widget.datas['id'],
    ...
    for(int i =0; i < dataOrder.length; i++){
      int ids = dataOrder[i]['id'];
      int users = dataOrder[i]['user_id'];
      for(int j =0; j < widget.datas['order'].length; j++){
        int idss = widget.datas['order'][j]['id'];
        int userss = widget.datas['order'][j]['user_id'];
        if(idss != ids && users != userss){
          news.add(dataOrder[i]);
        }
      }
    }
    dataBill.add(data);
                ...
                Padding(
                  ...
                  child: Row(
                    children: [
                      ...
                      Expanded(
                          child: Text(
                        "Rp. ${formatter.format(widget.datas['total'])}",
                ...
                Padding(
                  ...
                  child: Row(
                    children: [
                      ...
                      Expanded(
                          child: Text(
                        "Rp. ${formatter.format(widget.datas['service'])}",
                        textAlign: TextAlign.right,
                      ))
                    ],
                  ),
                ...
                Padding(
                  ...
                  child: Row(
                    children: [
                      ...
                      Expanded(
                          child: Text(
                        "Rp. ${formatter.format(widget.datas['tax'])}",
                        textAlign: TextAlign.right,
                      ))
                    ],
                  ),
